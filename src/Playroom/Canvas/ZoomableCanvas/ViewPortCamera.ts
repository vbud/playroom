...
export class ViewPortCamera {
  ...
  public centerFitHorizontalAreaIntoView(
    ...
    const updateTarget = !animationOptions
      ? this.workingValues
      : { ...this.workingValues };
    ViewPortMath.centerFitHorizontalArea(
      updateTarget,
      this.derivedBounds,
      left,
      width,
  ...
  public handleContainerSizeChanged(
    ...
    if (
      width === this.workingValues.containerWidth &&
      height === this.workingValues.containerHeight
    ) {
      return;
    }
    ...
    this.workingValues.width =
      this.workingValues.containerWidth / this.workingValues.zoomFactor;
  ...
  private handleAnimationFrame = (time: number) => {
    ...
    if (this.animation) {
      ...
      const completionPercent =
        (time - this.animation.startingTimeMilliseconds) /
        this.animation.durationMilliseconds;
  ...
  private scheduleAnimation(
    targetValues: ViewPortCameraValues,
    animationOptions: ViewPortCameraAnimationOptions
  ) {
    invariant(
      !this.animation,
      'Cannot schedule animation while another animation is still in progress.'
    );
