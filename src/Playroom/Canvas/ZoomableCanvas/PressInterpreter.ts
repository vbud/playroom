...
export class PressInterpreter {
  ...
  public readonly pressHandlers: Pick<
    ViewPortOptions,
    'onPressStart' | 'onPressMove' | 'onPressEnd' | 'onPressCancel'
  >;
  ...
  private handleCapturePressThresholdMet = () => {
    ...
    this.currentConfig?.onCapturePressStart?.(
      ...
      if (this.currentConfig.capturePressThresholdMs === 0) {
        ...
        if (this.currentConfig.longTapThresholdMs !== undefined) {
          this.longPressTimerId = setTimeout(
            ...
            this.currentConfig.longTapThresholdMs
          );
        }
      }
      return 'capture';
  ...
  private handlePressMove = (
    ...
    const xDelta = Math.abs(
      coordinates.containerX - this.currentPressStartingCoordinates.containerX
    );
    const yDelta = Math.abs(
      coordinates.containerY - this.currentPressStartingCoordinates.containerY
    );

    const maxDeltaAllowed =
      this.currentConfig.potentialTapBounds ?? POTENTIAL_TAP_BOUNDS_DEFAULT;
