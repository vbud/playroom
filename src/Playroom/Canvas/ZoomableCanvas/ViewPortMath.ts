...
export const ViewPortMath = {
  deriveActualZoomBounds(
    ...
    if (
      bounds?.x &&
      bounds.x?.[0] !== undefined &&
      bounds.x?.[1] !== undefined
    ) {
    ...
    if (
      bounds?.y &&
      bounds.y?.[0] !== undefined &&
      bounds.y?.[1] !== undefined
    ) {
      const space: VirtualSpacePixelUnit = bounds.y[1] - bounds.y[0];
      min = min
        ? Math.max(min, containerHeight / space)
        : containerHeight / space;
    }
    if (bounds?.zoom) {
  ...
  centerFitArea(
    ...
    let newZoomFactor = Math.min(
      zoomFactorBasedOnWidth,
      zoomFactorBasedOnHeight
    );
  ...
  updateBy(
    ...
    const oldVirtualSpaceVisibleSpaceWidth =
      values.containerWidth / values.zoomFactor;
    const oldVirtualSpaceVisibleSpaceHeight =
      values.containerHeight / values.zoomFactor;
    ...
    const virtualSpaceVisibleWidthDelta =
      values.width - oldVirtualSpaceVisibleSpaceWidth;
    const virtualSpaceVisibleHeightDelta =
      values.height - oldVirtualSpaceVisibleSpaceHeight;

    // The reason we use x and y here is to zoom in or out towards where the
    // pointer is positioned
    const xFocusPercent =
      anchorContainerX === undefined
        ? 0.5
        : anchorContainerX / values.containerWidth;
    const yFocusPercent =
      anchorContainerY === undefined
        ? 0.5
        : anchorContainerY / values.containerHeight;

    values.centerX = clampCenterOfLength(
      virtualSpaceNewLeft -
        virtualSpaceVisibleWidthDelta * xFocusPercent +
        values.width / 2,
      values.width,
      bounds?.x
    );
    values.centerY = clampCenterOfLength(
      virtualSpaceNewTop -
        virtualSpaceVisibleHeightDelta * yFocusPercent +
        values.height / 2,
      values.height,
      bounds?.y
    );
